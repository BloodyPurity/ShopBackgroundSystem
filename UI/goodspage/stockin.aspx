<%@ Page Language="C#" AutoEventWireup="true" CodeFile="stockin.aspx.cs" Inherits="UI_goodspage_stockin" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    ...
    <script>
        function subCheck() {
            var str = /^[0-9]+(.[0-9]{1,3})?$/;
            var goods = document.getElementById("ddlGoods");
            if (goods.value == null || goods.value == '') {
                alert("货物名为空");
                return false;
            }
            ...
            var count = document.getElementById("tbxCount").value;
            if (!str.test(count)) {
                alert("商品数量不正确");
                return false;
            }
...
<body>
    <form id="form1" runat="server">
        <div class="panel-footer"><h2>商品入库</h2></div>
        <div>
            <div class="col-md-2">
            ...
            <div class="col-md-10">
                ...
                <div class="col-md-9">
                    <div class="panel panel-default" title="我的入库">
                        <div class="panel-heading">
                            <h3 class=".text-info">我的入库</h3>
                        </div>
                        <asp:GridView ID="GridView1" runat="server"
                            ...
                            <Columns>
                                ...
                                <asp:BoundField DataField="pname" HeaderText="供应商" ReadOnly="True" />
                                <asp:BoundField DataField="count" HeaderText="入库量" />
                                <asp:BoundField DataField="percost" HeaderText="单价" />
                                <asp:BoundField DataField="cost" HeaderText="总价" />
                                <asp:BoundField DataField="notes" HeaderText="备注" />
                                <asp:BoundField DataField="ischecked" HeaderText="确认状态" />
                                <asp:TemplateField HeaderText="操作">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="btnEdit" Text="修改" CommandName="gedit" class="btn btn-primary" runat="server" CommandArgument='<%#Eval("id")%>'></asp:LinkButton>
                                        <asp:LinkButton ID="LinkButton1" Text="删除" CommandName="gdelete" class="btn btn-danger" runat="server" CommandArgument='<%#Eval("id")%>'></asp:LinkButton>
                                    </ItemTemplate>
                                </asp:TemplateField>
                            </Columns>
                        </asp:GridView>
                        <div class="panel-footer">注意：在订单未被店长确认时才可以修改。</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="panel panel-default" title="入库">
                        <div class="panel-heading">
                            <h3 class=".text-info">入库</h3>
                        </div>
                         <div class="panel-body">
                             <div class ="form-group">
                                 <label for="exampleInputPassword1">商品类型：</label>
                                 <asp:DropDownList ID="ddlGtype" class="form-control" runat="server" DataValueField="name" DataTextField="name" AutoPostBack="true" OnSelectedIndexChanged="ddlGtype_SelectedIndexChanged"></asp:DropDownList>
                             </div>
                             <div class="form-group">
                                <label for="exampleInputPassword1">供货商：</label>
                                <asp:DropDownList ID="ddlProvider" class="form-control" runat="server" DataValueField="name" DataTextField="name"></asp:DropDownList>
                              </div>
                                <div class="form-group">
                                    <label for="exampleInputEmail1">商品名：</label>
                                    <asp:DropDownList ID="ddlGoods" class="form-control" runat="server" DataValueField="gname" DataTextField="gname"></asp:DropDownList>
                                 </div>
                                <div class="form-group">
                              ...
                              <asp:Button ID="btnSubmitGoods" runat="server" Text="确定"  class="btn btn-primary" OnClick="btnSubmitGoods_Click"/>
                           </div>
